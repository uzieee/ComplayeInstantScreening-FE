  ...
  return (
    <div className="max-w-4xl space-y-4 md:space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg md:text-xl font-bold text-gray-900">Screening History</h2>
          <p className="text-xs md:text-sm text-gray-500">All searches performed by your organisation</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
        {/* Desktop header */}
        <div className="hidden md:grid grid-cols-[1fr_100px_80px_120px_44px] gap-3 px-5 py-3 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
          ...
          {data?.items?.map((item: any) => {
            ...
            return (
              <div key={item.session_id}>
                {/* Desktop row */}
                <div className="hidden md:grid grid-cols-[1fr_100px_80px_120px_44px] gap-3 px-5 py-3.5 items-center hover:bg-gray-50/60 transition-colors">
                ...
                {/* Mobile card */}
                <div className="md:hidden flex items-center gap-3 px-4 py-3 hover:bg-gray-50/60 transition-colors">
                  <span className={cn('w-8 h-8 rounded-lg flex items-center justify-center shrink-0', cfg.bg)}>
                    <Icon size={14} className={cfg.color} />
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-medium text-sm text-gray-900 truncate">{item.query_name}</p>
                      <div className="shrink-0">
                        {item.hit_count > 0
                          ? <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">{item.hit_count} hit{item.hit_count > 1 ? 's' : ''}</span>
                          : item.possible_count > 0
                          ? <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">{item.possible_count} possible</span>
                  ...
                  <button onClick={() => download(item.session_id)} className="text-gray-300 hover:text-[#F75835] transition-colors p-1 shrink-0">
                    <Download size={15} />
                  </button>
                </div>
              </div>
